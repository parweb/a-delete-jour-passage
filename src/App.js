...
class MainApp extends Component {
  ...
  getListCommunes = codeDepartement => {
    //var postalCodesJoin;
    var jsonBody;
    BCAServices.fetchCommunes(this.state.codePays, codeDepartement)
      .then(res => {
        if (res.ok) {
          jsonBody = res.body;
  ...
  getBureau = codesObject => {
    var jsonBody;
    BCAServices.fetchBureau(this.state.codePays, codesObject)
  ...
  handleFormSubmitted = formData => {
    ...
    if (formData.dataFromDepartement) {
      postalCodesJoin =
        formData.dataFromDepartement.numDepartement +
        formData.dataFromDepartement.numBureauDistrib;
      ...
      postalCodesSubString.push(
        postalCodesString.substring(postalCodesString.length - 3)
      );
  ...
  getJoursDePassage = params => {
    ...
    if (params !== undefined || Array.isArray(params)) {
      bcaServices
        .fetchJoursPassage(params[0], params[1])
        .then(res => {
          if (res.ok) {
            jsonBody = res.body;
            jsonArray = [jsonBody];
      ...
      bcaServices
        .fetchJoursPassage(this.state.codeDepartement, this.state.codeBureau)
        .then(res => {
          if (res.ok) {
            jsonBody = res.body;
            jsonArray = [jsonBody];
  ...
  render() {
    return (
      <div>
        <Button
          className="modal-expert bca-btn"
          variant="primary"
              ...
              <Modal.Body>
                <div className="popup-expert-main">
                  ...
                  <Container>
                    ...
                    {this.state.notFound && (
                      <span>
                        le code postal {this.state.postalCodeDisplay} ne
                        correspond pas à un bureau distributeur
                      </span>
                    )}
                  </Container>
                  <Container>
