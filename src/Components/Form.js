...
export default function FormBca(props) {
  ...
  const [disableSelectDepartement, setDisableSelectDepartement] = useState(
    false
  );
  ...
  useEffect(() => {
    ...
    postalInput
      ? setDisableSelectDepartement(true)
      : setDisableSelectDepartement(false);
  ...
  function formatDepartementNumber(numero) {
    ...
    numero < 10
      ? (stringFormated = '0' + numero + hundredsString)
      : (stringFormated = numero + hundredsString);
    return stringFormated;
  }

  function isDataPostalHelper() {
    if (disableInputPostal) return false;
    else return true;
  }
    ...
    <Row>
      ...
      >
        <FormGroup as={Row}>
          ...
          <Col md="2">
            <FormControl
              required={!disableInputPostal}
              id="formGriPostalCode"
              disabled={disableInputPostal}
              minLength={5}
              maxLength={5}
              pattern="[0-9]+$"
              name="codePostal"
          ...
          <Col md="4">
            <FormControl
              ref={selectDepartement}
              id="formGriDepartement"
              disabled={disableSelectDepartement}
              required={disableInputPostal || postalInput === ''}
            ...
            >
              ...
              {departmentsList.length > 0 &&
                departmentsList.map((items, index) => {
                  return (
                    <option key={index} value={items.numero}>
                      {items.libelle} ({formatDepartementNumber(items.numero)})
                    </option>
                  );
                })}
            </FormControl>
          ...
          {!displayVille && (
            <Col md="2">
              <Button
                className="bca-btn form-sub-btn clear-inp"
            ...
            <Col md="8">
              ...
              >
                ...
                {communesList.length > 0 &&
                  communesList.map((items, index) => {
                    return (
                      <option key={index} value={items.numBureauDistrib}>
                        {items.libelle}
                      </option>
                    );
                  })}
        ...
        <FormGroup as={Row} controlId="formGridReset">
          <Col>
            <Button className="bca-btn form-sub-btn" type="submit">
              ...
              {screenResponsive != null &&
                (screenResponsive < 480 || screenResponsive > 1024) &&
                'Recherche'}
            </Button>
          </Col>
        </FormGroup>
      ...
      {displayWarning && (
        <span>
          Veuillez saisir un code postal ou sélectionner un département et une
          localité
        </span>
      )}
    </Row>
